...
JNIEXPORT JNICALL jint TNN_OBJECT_DETECTOR_NANODET(nativeInit)(JNIEnv *env, jobject thiz, jstring modelPath, jint computUnitType) {
    ...
    if (clsObjectInfo == NULL) {
        clsObjectInfo = static_cast<jclass>(env->NewGlobalRef(env->FindClass("com/ubeesky/lib/ai/AIDetectResult")));
        midconstructorObjectInfo = env->GetMethodID(clsObjectInfo, "<init>", "()V");
        fidx1 = env->GetFieldID(clsObjectInfo, "startX", "F");
        fidy1 = env->GetFieldID(clsObjectInfo, "startY", "F");
        fidx2 = env->GetFieldID(clsObjectInfo, "endX", "F");
        fidy2 = env->GetFieldID(clsObjectInfo, "endY", "F");
        fidscore = env->GetFieldID(clsObjectInfo, "confidence", "F");
        fidlandmarks = env->GetFieldID(clsObjectInfo, "landmarks", "[F");
        fidcls = env->GetFieldID(clsObjectInfo, "classId", "I");
    }
...
//            LOGI("object[%d] %f %f %f %f score %f landmark size %d, label_id: %d", i, objectInfoList[i].x1, objectInfoList[i].y1, objectInfoList[i].x2, objectInfoList[i].y2, objectInfoList[i].score,
//                 landmarkNum, objectInfoList[i].class_id);
...
JNIEXPORT JNICALL jobjectArray TNN_OBJECT_DETECTOR_NANODET(nativeDetectFromImage)(JNIEnv *env, jobject thiz, jobject imageSource, jint width, jint height) {
    // TODO: implement detectFromImage()
    return nullptr;
}