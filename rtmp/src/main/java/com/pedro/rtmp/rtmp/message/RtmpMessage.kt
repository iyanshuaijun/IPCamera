...
abstract class RtmpMessage(basicHeader: BasicHeader) {
    ...
    companion object {
        ...
        fun getRtmpMessage(
            input: InputStream, chunkSize: Int,
            commandSessionHistory: CommandSessionHistory
        ): RtmpMessage {
        ...
        fun getMarkType(type: Int): MessageType {
            return MessageType.values().find { it.mark.toInt() == type }
                ?: throw IOException("Unknown rtmp message type: $type")
        }

        private fun getInputWithoutChunks(
            input: InputStream, header: RtmpHeader, chunkSize: Int,
            commandSessionHistory: CommandSessionHistory
        ): InputStream {
    ...
    fun writeBody(output: OutputStream) {
        ...
        while (length > chunkSize) {
            ...
            header.writeHeader(
                BasicHeader(ChunkType.TYPE_3, header.basicHeader.chunkStreamId),
                output
            )
        }
