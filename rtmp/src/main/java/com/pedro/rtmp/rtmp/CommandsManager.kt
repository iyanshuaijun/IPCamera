...
class CommandsManager {
    ...
    fun sendConnect(auth: String, socket: RtmpSocket) {
        synchronized(writeSync) {
            val output = socket.getOutStream()
            val connect = CommandAmf0(
                "connect", ++commandId, getCurrentTimestamp(), streamId,
    ...
    fun createStream(socket: RtmpSocket) {
        synchronized(writeSync) {
            val output = socket.getOutStream()
            val releaseStream = CommandAmf0(
                "releaseStream", ++commandId, getCurrentTimestamp(), streamId,
            ...
            val fcPublish = CommandAmf0(
                "FCPublish", ++commandId, getCurrentTimestamp(), streamId,
                BasicHeader(ChunkType.TYPE_0, ChunkStreamId.OVER_STREAM.mark)
            )
            ...
            val createStream = CommandAmf0(
                "createStream", ++commandId, getCurrentTimestamp(), streamId,
                BasicHeader(ChunkType.TYPE_0, ChunkStreamId.OVER_CONNECTION.mark)
            )
    ...
    fun sendPublish(socket: RtmpSocket) {
        synchronized(writeSync) {
            val output = socket.getOutStream()
            val name = "publish"
            val publish = CommandAmf0(
                name, ++commandId, getCurrentTimestamp(), streamId,
                BasicHeader(ChunkType.TYPE_0, ChunkStreamId.OVER_STREAM.mark)
            )
