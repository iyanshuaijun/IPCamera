...
open class ServerCommandManager(
            ...
            Method.SETUP -> {
                ...
                if (track != null) {
                    ...
                    return when (protocol) {
                        ...
                        Protocol.UDP -> {
                            if (loadPorts(request, track)) createSetup(
                                cSeq,
                                track
                            ) else createError(500, cSeq)
                        }
                        else -> {
    ...
    private fun getTrack(request: String): Int? {
        val trackMatcher =
            Pattern.compile("streamid=(\\w+)", Pattern.CASE_INSENSITIVE).matcher(request)
    ...
    private fun createBody(): String {
        ...
        var videoBody = ""
        if (!videoDisabled) {
            videoBody = if (vps == null) SdpBody.createH264Body(
                RtpConstants.trackVideo,
                encodeToString(sps!!)!!,
                encodeToString(pps!!)!!
            )
            else SdpBody.createH265Body(
                RtpConstants.trackVideo,
                encodeToString(sps!!)!!,
                encodeToString(pps!!)!!,
                encodeToString(vps!!)!!
    ...
    private fun createSetup(cSeq: Int, track: Int): String {
        val protocolSetup = if (protocol == Protocol.UDP) {
            ...
            val serverPorts =
                if (track == RtpConstants.trackAudio) audioServerPorts else videoServerPorts
