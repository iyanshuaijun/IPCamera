 ...
 * RFC 3984
 */
open class H264Packet(
    sps: ByteArray,
    pps: ByteArray,
    private val videoPacketCallback: VideoPacketCallback
) : BasePacket(
    ...
    override fun createAndSendPacket(byteBuffer: ByteBuffer, bufferInfo: MediaCodec.BufferInfo) {
        ...
        if (type == RtpConstants.IDR || bufferInfo.flags == MediaCodec.BUFFER_FLAG_KEY_FRAME) {
            stapA?.let {
                ...
                updateSeq(buffer)
                val rtpFrame = RtpFrame(
                    buffer,
                    rtpTs,
                    it.size + RtpConstants.RTP_HEADER_LENGTH,
                    rtpPort,
                    rtcpPort,
                    channelIdentifier
                )
        ...
        if (sendKeyFrame) {
            ...
            if (naluLength <= maxPacketSize - RtpConstants.RTP_HEADER_LENGTH - 1) {
                ...
                updateSeq(buffer)
                val rtpFrame =
                    RtpFrame(buffer, rtpTs, buffer.size, rtpPort, rtcpPort, channelIdentifier)
                videoPacketCallback.onVideoFrameCreated(rtpFrame)
            } else {
                // Set FU-A header
                ...
                while (sum < naluLength) {
                    val length =
                        if (naluLength - sum > maxPacketSize - RtpConstants.RTP_HEADER_LENGTH - 2) {
                    ...
                    if (sum >= naluLength) {
                        // End bit on
                        buffer[RtpConstants.RTP_HEADER_LENGTH + 1] =
                            buffer[RtpConstants.RTP_HEADER_LENGTH + 1].plus(0x40).toByte()
                        markPacket(buffer) //mark end frame
                    }
                    updateSeq(buffer)
                    val rtpFrame =
