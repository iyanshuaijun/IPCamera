...
class H264PacketTest {
    ...
    fun `GIVEN a small ByteBuffer raw h264 WHEN create a packet THEN get a RTP h264 packet`() {
        ...
        val expectedRtp = byteArrayOf(
            ...
            21,
            5
        ).plus(fakeH264.copyOfRange(header.size, fakeH264.size))
        val expectedStapA = byteArrayOf(
            -128,
            -32,
            0,
            1,
        ...
        val expectedStapAResult = RtpFrame(
            expectedStapA,
            expectedTimeStamp,
            fakePps.size + fakePps.size + 5 + RtpConstants.RTP_HEADER_LENGTH,
            1,
            2,
            RtpConstants.trackVideo
        )
        val expectedPacketResult =
            RtpFrame(expectedRtp, expectedTimeStamp, expectedSize, 1, 2, RtpConstants.trackVideo)
    ...
    fun `GIVEN a big ByteBuffer raw h264 WHEN create a packet THEN get a RTP h264 packet`() {
        ...
        val expectedRtp =
            byteArrayOf(-128, 96, 0, 2, 0, -87, -118, -57, 7, 91, -51, 21, 28, -123).plus(chunk1)
        val expectedRtp2 =
            byteArrayOf(-128, -32, 0, 3, 0, -87, -118, -57, 7, 91, -51, 21, 28, 69).plus(chunk2)
        ...
        val expectedPacketResult =
            RtpFrame(expectedRtp, expectedTimeStamp, expectedSize, 1, 2, RtpConstants.trackVideo)
        val expectedPacketResult2 =
            RtpFrame(expectedRtp2, expectedTimeStamp, expectedSize2, 1, 2, RtpConstants.trackVideo)
